...
export default function Incidents() {
  ...
  const load = () => client.get("/incidents/").then((res) => setIncidents(res.data));
  useEffect(load, []);
  ...
  return (
    <div style={{ padding: "1rem", maxWidth: 700 }}>
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: "1rem" }}>
        ...
        <label>Reliability (1-5): <input type="number" min={1} max={5} value={form.reliability_rating} onChange={(e) => setForm({ ...form, reliability_rating: parseInt(e.target.value) })} /></label>
        <button type="submit">Log Incident</button>
      </form>
      <ul>
        {incidents.map((i) => (
          <li key={i.id}>{i.incident_date?.slice(0, 10)} — {i.location} — {i.type} (reliability {i.reliability_rating}/5)</li>
        ))}
      </ul>
    </div>
  );
